...
describe("API Tests", () => {
  ...
  it("POST /api/users should create a new user", () => {
    ...
    cy.request("POST", `${Cypress.env("baseUrl")}/api/users`, newUser).then(
      (response) => {
  ...
  it("GET /api/users should retrieve the created users", () => {
    ...
    cy.request("GET", `${Cypress.env("baseUrl")}/api/users`).then(
      (response) => {
        expect(response.status).to.eq(200);
        // Check that the response body is an array
        expect(response.body).to.be.an("array");
        // Check that each object in the array is defined
        cy.wrap(response.body).each((user) => {
          expect(user).to.not.be.undefined;
        });

        // Verify if the retrieved user is present in the array
        const userInArray = response.body.find(
          (user: { _id: string }) => user._id === originalUserId
        );
  ...
  it("PUT /api/users should update the created user", () => {
    ...
    cy.request(
      "PUT",
      `${Cypress.env("baseUrl")}/api/users/?userID=${originalUserId}`,
      updatedUserData
    ).then((response) => {
      expect(response.status).to.eq(200);
    ...
    cy.request(
      "GET",
      `${Cypress.env("baseUrl")}/api/users/${originalUserId}`
    ).then((response) => {
      expect(response.status).to.eq(200);
  ...
  it("GET /api/userfilter should retrieve the specific user based on filters", () => {
    ...
    cy.request("GET", `${Cypress.env("baseUrl")}/api/userfilter`, {
      qs: filters,
    })
      .then((response) => {
        expect(response.status).to.eq(200);
  ...
  it("DELETE /api/users should delete the created user", () => {
    ...
    cy.request(
      "DELETE",
      `${Cypress.env("baseUrl")}/api/users/?userID=${userIdToDelete}`
    ).then((response) => {
      expect(response.status).to.eq(200);
    });
    ...
    cy.request("GET", `${Cypress.env("baseUrl")}/api/users`).then(
      (response) => {
        ...
        const deletedUser = response.body.find(
          (user: any) => user._id === userIdToDelete
        );
  ...
  it("POST /api/property should create a new property", () => {
    ...
    cy.request(
      "POST",
      `${Cypress.env("baseUrl")}/api/property`,
      newProperty
    ).then((response) => {
      expect(response.status).to.eq(200);
      expect(response.body).to.have.property("_id");
  ...
  it("GET /api/property should retrieve the created property by propertyID", () => {
    ...
    cy.request("GET", `${Cypress.env("baseUrl")}/api/property`).then(
      (response) => {
        expect(response.status).to.eq(200);
        expect(response.body).to.be.an("array");

        // Verify if the retrieved property is present in the array
        const propertyInArray = response.body.find(
          (property: { _id: string }) => property._id === originalPropertyId
        );
  ...
  it("PUT /api/property should update the created property", () => {
    ...
    cy.request(
      ...
      `${Cypress.env(
        "baseUrl"
      )}/api/property/?propertyId=${originalPropertyId}`,
      updatedPropertyData
    ).then((response) => {
      expect(response.status).to.eq(200);
    ...
    cy.request(
      "GET",
      `${Cypress.env("baseUrl")}/api/property/${originalPropertyId}`
    ).then((response) => {
      expect(response.status).to.eq(200);
  ...
  it("GET /api/propertyfilter should retrieve the specific property based on filters", () => {
    ...
    cy.request("GET", `${Cypress.env("baseUrl")}/api/propertyfilter`, {
      qs: filters,
    })
      .then((response) => {
        expect(response.status).to.eq(200);
        ...
        const matchingProperty = response.body.find(
          (updatedPropertyData: {
            propertytype: any;
            address: string;
            saletype: string;
  ...
  it("DELETE /api/property should delete the created property", () => {
    ...
    cy.request(
      "DELETE",
      `${Cypress.env("baseUrl")}/api/property/?propertyId=${originalPropertyId}`
    ).then((response) => {
      expect(response.status).to.eq(200);
    });
    ...
    cy.request("GET", `${Cypress.env("baseUrl")}/api/property`).then(
      (response) => {
        ...
        const deletedProperty = response.body.find(
          (property: any) => property._id === originalPropertyId
        );
  ...
  it("should add an offer", () => {
    ...
    it("should delete an offer", () => {
      ...
      cy.request(
        "DELETE",
        `${Cypress.env("baseUrl")}/api/offerdelete/?offerId=${offerId}`
      ).then((response) => {
        expect(response.status).to.eq(200);
        ...
        if (response.body === null || response.body === undefined) {
          ...
          throw new Error(
            "DELETE request returned an unexpected response body"
          );
        }
      });
    });
    ...
    it("should log in, navigate to the calculator, and calculate monthly payment", () => {
      ...
      cy.visit("https://5ftapart.vercel.app/login");
      ...
      cy.visit("https://5ftapart.vercel.app/calculator");
      ...
      cy.get(".justify-center > .gap-2 > :nth-child(3) > .mt-2")
        .clear()
      ...
      cy.get(":nth-child(2) > .mt-2")
        .clear()
        .type(invalidInterestRate.toString());
      cy.get(".justify-center > .gap-2 > :nth-child(3) > .mt-2")
        .clear()
        .type(invalidPaymentFrequency.toString());
