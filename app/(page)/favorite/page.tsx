...
} from "../../../backend/lib/helperFavorite";
...
const FavoritePage = () => {
  ...
  const handleUnfavorite = async(property_id: any) => {
    try {
      ...
      const updatedFavorites = favorites.filter(item => item._id !== property_id);
  ...
  return (
    <main className={`${styles.main} flex-grow`}>
      ...
      {favorites.length > 0 ? ( <div className="result-content p-5 grid grid-cols-3 gap-x-4 gap-y-8 mx-72">
