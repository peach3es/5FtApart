...
function AddBrokerForm({formData, setFormData}){
    ...
    const handleSubmit = (e) =>{
        ...
        setError(null);
        let {firstname, lastname, email, password, date, activeListings} = formData;
        ...
        const model = {
            ...
            email, password, date, activeListings
        }
    ...
    useEffect(() => {
        let timeoutId;
        if (addMutation.isSuccess || isEmpty) { // Check either for mutation success or data empty
    ...
    }, [addMutation.isSuccess, isEmpty]); // Add isEmpty as a dependency
    ...
    return (
        ...
        <div className="input-type">
            <input type="password" onChange={setFormData} name="password" className="border w-full px-5 py-3 focus:outline-none rounded-md" placeholder="Password" />
        </div>
        <div className="input-type">
            <input type="date" onChange={setFormData} name="date" className="border w-full px-5 py-3 focus:outline-none rounded-md" placeholder="Salary" />
        </div>

        <div className="input-type">
            <input type="number" onChange={setFormData} name="activeListings" className="border w-full px-5 py-3 focus:outline-none rounded-md" placeholder="Active Listings" />
        </div>
        ...
        <button type="submit" className="flex justify-center text-md w-full bg-green-500 text-white px-4 py-5 border rounded-md hover:bg-gray-50 hover:border-green-500 hover:text-green-500 text-xl">
         Add <span className="px-1"><BiPlus size={28}></BiPlus></span>
        </button>
